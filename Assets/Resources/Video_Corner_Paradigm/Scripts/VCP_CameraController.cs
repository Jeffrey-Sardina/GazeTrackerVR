...
public class VCP_CameraController : MonoBehaviour
{
    ...
    static readonly Vector3 DISPLAY_LOCATION = new Vector3(0, 0, 10),
        QI = DISPLAY_LOCATION + new Vector3(9, 7, 0),
        QII =DISPLAY_LOCATION + new Vector3(-9, 7, 0),
        QIV = DISPLAY_LOCATION + new Vector3(9, -7, 0),
        QIII = DISPLAY_LOCATION + new Vector3(-9, -7, 0),
        OFFSET = new Vector3(0, 0, -0.05f);
    const string PARADIGM_PATH = "Video_Corner_Paradigm/",
        DATA_EXTENSION = ".csv",
        EXPERIMENT_FILE_BASE_NAME = "VCP_",
    ...
    void OnVideoOver(VideoPlayer vp)
    {
        ...
        if (fixationDot == null)
        {
            fixationDot = DisplayFixationDot(DISPLAY_LOCATION + OFFSET, FIXATION_DOT);
        }
    }
    ...
    void OnReadyDone()
    {
        ...
        videoPlane = DisplayVideo(STIMULUS_LIST[currentStimuliListIndex][4], DISPLAY_LOCATION + OFFSET, IMAGE_SCALE);
        Destroy(fixationDot);
        fixationDot = null;
    ...
    void RunExperiment()
    {
        ...
        if (fixationDotCallibrationDone
            ...
            && !imageInitialDispayDone)
        {
            readyDot = DisplayFixationDot(DISPLAY_LOCATION + OFFSET, READY_DOT);
    ...
    bool RunStartUp()
    {
        if (videoPlane == null)
        {
            videoPlane = DisplayVideo("VocabInstructions", DISPLAY_LOCATION + OFFSET, IMAGE_SCALE * 3);
            fixationDot = DisplayFixationDot(DISPLAY_LOCATION + new Vector3(0f, 3f, 0f) + OFFSET, FIXATION_DOT);
        }
